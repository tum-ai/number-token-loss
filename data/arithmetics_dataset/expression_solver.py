...
class ExpressionSolver:
    ...
    def __init__(self, operators: dict[str, Callable[[int, int], int]] = None):
        """
    ...
    def solve(self, expression: str) -> dict[str, Any]:
        ...
        """
        result, steps = self._convert_to_steps_and_result(expression)
        context = f"\n\nQ: What is {expression}?\n\nA: "
        completion = " ".join(f"<<{step}>>" for step in steps) + f" {result}"
        solution = {
            "context": context,
            "path": [f"<<{step}>>" for step in steps],
            "solution": str(result),
